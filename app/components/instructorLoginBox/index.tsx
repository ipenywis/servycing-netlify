...
const ActionsDispatcher = (dispatch: Dispatch) => ({
  instructorAuthenticated: (token: string) =>
    dispatch(instructorAuthenticated(token)),
});

export function InstructorLoginBox(props: ILoginBoxProps) {
  const [error, setError] = useState<string | null>(null);

  const history = useHistory();

  const { instructorAuthenticated } = ActionsDispatcher(useDispatch());

  const onSubmit = async (values: any, form: FormApi<any>): Promise<any> => {
    setError(null);

    const data: IInstructorLoginDTO = {
      username: values.username,
      password: values.password,
    };
    const instructor = await instructorService.login(data).catch(err => {
      setError(err.message);
    });

    if (instructor) {
      instructorAuthenticated(instructor.access_token as string);
      history.push(ROUTES.instructorDashboad);
    }
  };

  return (
    <LoginCard title="Instructor Login" centerTitle>
      <GithubButton
